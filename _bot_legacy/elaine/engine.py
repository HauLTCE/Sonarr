...
class Engine:
    ...
    def _apply_one_set(self, sa: SetAction, mr: MatchResult) -> None:
        ...
        if sa.op == "rand":
            ...
            return
        if sa.op == "pick":
            # Deterministic choice from a fixed list of literals, seeded like `rand`
            # (turn ^ key) so it's stable within a turn and replayable. Used to assign and
            # store a varied placeholder (e.g. a nickname when no real name was given).
            seed = self.memory.turn ^ zlib.crc32(sa.key.encode("utf-8"))
            self.memory.set(sa.key, random.Random(seed).choice(sa.value), ttl=sa.ttl)
            return
        if sa.op == "put":
