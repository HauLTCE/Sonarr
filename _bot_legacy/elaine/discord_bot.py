...
class ElaineBot:
    ...
    def handle_message(self, msg: IncomingMessage) -> str | None:
        ...
        with lock:
            ...
            # Revive from a terminal farewell. BYE is `end: true`, so once a user says "bye"
            # the engine's step() short-circuits to an empty no-op halt. In the CLI that's
            # fine (the REPL exits on halt and never re-steps), but Discord is a persistent
            # per-user relationship saved/reloaded every message — without this the next
            # message hits permanent silence (reply=None). Re-enter CHAT (a name is always
            # set by the time BYE is reachable) so she picks the conversation back up.
            cur = ae.engine.current
            if cur in self.loaded.states and self.loaded.states[cur].end:
                revive = "CHAT" if "CHAT" in self.loaded.states else self.loaded.start
                logger.info("handle: %s reviving from terminal %s -> %s", key, cur, revive)
                ae.engine.current = revive
                ae.self_state.dialogue_state = revive
