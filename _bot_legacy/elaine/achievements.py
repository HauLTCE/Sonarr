...
# Names the BOT assigns when the user won't give one (GREET DENY / fallback `pick` lists),
# plus the legacy "friend" default for backward compatibility with old saved states. Being
# stuck with one of these is NOT a real introduction, so it must not unlock "Introduced".
# Keep in sync with the pick lists in persona/elaine.yaml.
_PLACEHOLDER_NAMES = frozenset({
    "friend", "trouble", "stranger", "nobody", "mystery", "tagalong", "rando",
})
...
_RULES = [
    ("hello", "Introduced", "told her your name",
     lambda e: e.memory.has("name") and e.memory.get("name") not in _PLACEHOLDER_NAMES),
